...
for ch = 1:size(MUA,2)
    ...
    spike_index(spike_index < 21) = [];
    spike_index(spike_index > size(MUA,1) - 21) = [];
    spike_times = (spike_index - 1)/fsMUA;

    %% spikes shape 
    spikes_shape = zeros(41,length(spike_index));
    for j = 1:length(spike_index)
        spikes_shape(:,j) = MUA(spike_index(j)-20:spike_index(j)+20);
    end
